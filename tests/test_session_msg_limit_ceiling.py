...
def test_frontend_reload_width_paths_read_the_live_ceiling():
    ...
    assert "numericLimit>0&&numericLimit<=_msgLimitMax" in _SESSIONS_JS  # shared reload URL policy
    assert "_sessionMessageReloadUrl(sid,reloadLimit)" in _SESSIONS_JS
