...
function loadSession(sid) {
  ...
  // Extension pre-open hook — fire once for the canonical navigation intent.
  // Coordinator follow-ups and internal redirects call _loadSessionOnce()
  // directly, so they cannot re-veto an already accepted session open.
  ...
  try {
    ...
    if(profileMismatch && profileMismatch.profile && !opts.skipProfileResolve){
      ...
      try{
        ...
        return _loadSessionOnce(sid,{...opts,skipProfileResolve:true,force:true,_preloadNotified:true});
      }catch(switchErr){
        e=switchErr;
      }
    }
